 ...
 */
const batchedFetch = (request) => new Promise((resolve) => {
...
const debounceAndBatch = (fn) => {
    ...
    return (value) => {
        args.push(value);
        if (!timeout)
            timeout = setTimeout(process, BATCH_WINDOW_SIZE_IN_MS);
    };
};
const debouncedFetch = debounceAndBatch(async (requests) => {
    // inject all request bodies into first request
    const allRequestBodies = await Promise.all(requests.map(({ request }) => request.json()));
